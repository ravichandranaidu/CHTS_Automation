...
public class ReportGenerator implements FileConstants
{
    ...
    public static String generateSummaryTable(ArrayList<TestSummaryResult> testsuites) throws Exception
    {
        ...
        for (Iterator<TestSummaryResult> iterator = testsuites.iterator(); iterator.hasNext();)
        {
            ...
            content = content + "<tr><td>" + testSuite.getSuiteName() + "</td><td>" + testSuite.getTotal()
                    + "</td><td>" + testSuite.getPassCount() + "</td><td><font color= \"" + color + "\">"
                    + testSuite.getFailCount() + "</font color></td><td>" + testSuite.getPercentage() + "%</td></tr>";
    ...
    public static ArrayList<TestCaseDetail> getAllTestCasesDetailreport()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            if (i == 0)
            {
                ...
                TestCaseDetail testSuite = new TestCaseDetail("Login", "Login Password", "Error Msseage thrown",
                        "Pass", "alsk", "a,SNc");
                testsuites.add(testSuite);
            }
            if (i == 1)
